...
aba1, aba2, aba3, aba4, aba5, aba6, aba7 = st.tabs(["Geral","Datas","Coop. Ativos","Emprestimo",
    "Depositos(Total)","Média Cooperados e Media de Emprestimos", 
...
with aba7:
    ...
    #Tabela com a média de cooperados ativos em um período de 19 dias, ou seja, a soma total de 
        #cooperados dividida por 19 dias.
    with coluna2:
        st.metric("Media de Coop. Ativos", format_number( ( df["Coop. Ativos"].sum() ) / df.shape[0] ), "R$" )

    #Tabela da média dos empréstimos concedidos em um período de 19 dias, ou seja, a soma total de 
        #todos os empréstimos dividida por 19 dias.
    with coluna3:
        st.metric("Media de Emprestimo", format_number( ( df["Emprestimo"].sum() ) / df.shape[0] ), "R$" )
